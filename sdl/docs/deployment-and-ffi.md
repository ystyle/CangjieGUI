...
部分 Linux 发行版上，系统 `libglib-2.0.so.0`（`SDL3_ttf` 的间接依赖）编译时链接了带版本符号的
PCRE2（如 `@PCRE2_10.47`），而仓颉运行时自带的 `libpcre2-8.so.0` 不带版本符号。链接器若优先找到
运行时自带的 PCRE2，会导致大量 `undefined reference to ...@PCRE2_10.47` 链接错误。

**解决方法**：在可执行程序的 `cjpm.toml` 中通过 `link-option` 显式指定系统 PCRE2：
