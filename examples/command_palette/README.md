...
一个键盘驱动的模糊命令启动器（类似编辑器的 Ctrl+P），集中演示框架累积的交互能力：
自动聚焦搜索、派生结果列表、以及“焦点在搜索框、方向键却操控列表”的经典命令面板交互。

## 你将学到

- `TextField.autofocus()`：面板一出现就聚焦搜索框，打开即可键入
- **应用级键盘路由**：为什么 `↑/↓/回车` 要在应用根部（`EventHandler`）拦截，而不是靠列表自身聚焦
- 结果列表是查询的**纯函数**（`filterCommands`），无需缓存；`State.observe` 在查询变化时归零高亮
...
命令面板的精髓是：**始终在搜索框输入**，同时用方向键在结果间移动、回车执行。若依赖 `ListView`
自身的键盘导航，就得先把焦点从搜索框移到列表，破坏“边打字边选择”。因此这里在应用根部拦截：
...
`EventHandler` 的 `onEvent` 先于子树运行，返回 `true` 即消费该事件——于是 `↑/↓/回车` 不会到达
搜索框，而其余按键（含撤销）照常交给搜索框处理。

### 结果是查询的纯函数

模型不缓存结果列表：`results()` 每次都由当前 `query` 现算。高亮索引在查询变化时归零，
保证键入后总是选中最匹配的首项：
...
每条命令带一组小写英文别名，匹配对标题按原样、对别名按小写，因此中文标题也能被英文关键词命中：
