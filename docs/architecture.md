...
转发），`.enabled(false)` 据此把该 id 移出当帧焦点环。包裹多个可聚焦控件的容器没有单一 id，其子树
停靠点保持不变——按控件粒度禁用。
...
仍能照常收到。控件请求的是语义形状 `CursorShape`（`Default` / `Interactive` / `Text`）而非平台
...
弹出列表里的点击会被其下方的控件截走。`UiContext` 因此持有一个 `Overlay`（`handleEvent` + `render`
两个闭包）：外壳每帧在树之前清空该槽、打开中的控件在 `draw` 里重新登记（闭包捕获其保留状态与弹出
矩形）、外壳在事件分发时**先把事件交给浮层**（消费则不再下发给树）、并在树之后绘制浮层。开合状态是
控件保留的 `open` 标志，与浮层的一帧延迟登记天然协作：本帧 `draw` 登记的浮层供**下一帧**事件使用，
关闭时控件 `clearActiveOverlay` 立即撤下。`Dropdown` 与 `ContextMenu` 都建立在这套机制上：前者点击展开
锚定在控件下方的选择列表，后者右键在指针处弹出动作菜单——两者都借由“事件先于树”解决了“弹出内容
被下方控件截走点击”这一经典难题。同一机制还可承载组合框下拉、子菜单等。为支持自定义外壳，
`dispatchOverlay`/`drawActiveOverlay` 也对外公开。
...
`content.handle`、并整体拦截以隔离背景，`render` 先画暗化遮罩再画面板与内容。因此一个浮层槽既能承载
手绘弹层，也能承载带自身状态的控件子树。`Modal` 自身零尺寸、仅在呈现时构建 `body`（隐藏对话框不注册
可聚焦项、不占布局），置于根部 `ZStack` 即对视口居中——默认隐藏故静态快照不变。
...
`TextArea` 各持有一个 `ScrollBar` 实例并转发指针事件，因此三者的滑块拖动、轨道翻页、拖动高亮完全同源，
不存在三份各自演化的实现。`ListView` 另实现键盘导航：注册进焦点环后可 `Tab` 聚焦，方向键在钳制范围内
